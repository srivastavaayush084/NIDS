#!/usr/bin/env python
"""
CLI Script to Evaluate Trained Dense Autoencoder Anomaly Detector.

Usage:
    python scripts/evaluate_autoencoder.py --dataset synthetic
    python scripts/evaluate_autoencoder.py --dataset nsl_kdd --unseen-attacks u2r,r2l
    python scripts/evaluate_autoencoder.py --dataset cicids2017 --unseen-attacks botnet,infiltration
    python scripts/evaluate_autoencoder.py --dataset unsw_nb15 --unseen-attacks worms,shellcode
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ZeroDayAI - Dense Autoencoder Anomaly Detection Evaluation",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom path to trained Autoencoder .pt file.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
...
def main():
    args = parse_args()
    logger.info("=" * 60)
    logger.info(f"STARTING AUTOENCODER EVALUATION: dataset='{args.dataset}'")
    logger.info("=" * 60)
    ...
    model_path = (
        ...
        if args.model_path
        else settings.MODELS_DIR / f"autoencoder_{args.dataset}_v{args.version}.pt"
    )

    if not model_path.exists():
        logger.error(
            f"Trained model not found at {model_path}. "
            f"Please run: python scripts/train_autoencoder.py --dataset {args.dataset}"
        )
        sys.exit(1)

    detector = AutoencoderDetector.load(model_path)
    ...
    print("\n" + "=" * 60)
    print(f"AUTOENCODER EVALUATION RESULTS ({args.dataset.upper()})")
    print("=" * 60)
