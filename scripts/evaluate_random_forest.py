#!/usr/bin/env python
"""
CLI Script to Evaluate Trained Supervised Random Forest Baseline Classifier.

Usage:
    python scripts/evaluate_random_forest.py --dataset synthetic
    python scripts/evaluate_random_forest.py --dataset nsl_kdd --unseen-attacks u2r,r2l
    python scripts/evaluate_random_forest.py --dataset cicids2017 --unseen-attacks botnet,infiltration
    python scripts/evaluate_random_forest.py --dataset unsw_nb15 --unseen-attacks worms,shellcode
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ZeroDayAI - Supervised Random Forest Baseline Evaluation",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom path to trained Random Forest .joblib file.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=None,
        help="Custom attack decision threshold (0.0 to 1.0). Defaults to learned threshold.",
    )
    parser.add_argument(
        "--unseen-attacks",
        type=str,
...
def main():
    args = parse_args()
    logger.info("=" * 60)
    logger.info(f"STARTING RANDOM FOREST EVALUATION: dataset='{args.dataset}'")
    logger.info("=" * 60)
    ...
    model_path = (
        ...
        if args.model_path
        else settings.MODELS_DIR / f"random_forest_{args.dataset}_v{args.version}.joblib"
    )

    if not model_path.exists():
        logger.error(
            f"Trained model not found at {model_path}. "
            f"Please run: python scripts/train_random_forest.py --dataset {args.dataset}"
        )
        sys.exit(1)
    ...
    print("\n" + "=" * 60)
    print(f"RANDOM FOREST BASELINE EVALUATION RESULTS ({args.dataset.upper()})")
    print("=" * 60)
    print(f"Total Test Samples:          {report['test_samples_total']} (Normal: {report['class_distribution']['normal']}, Attack: {report['class_distribution']['attack']})")
    ...
    print("-" * 60)
    print("Zero-Day & Attack Category Breakdown (Supervised Baseline Response):")
    ...
    print("-" * 60)
    print("Top Feature Importances (Gini):")
    for feat in report["top_feature_importances"][:8]:
        print(f"  #{feat['rank']:<2} - {feat['feature']:<25}: {feat['importance']:.4f}")
    print("-" * 60)
